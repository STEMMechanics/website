...
<div class="grid gap-x-10 gap-y-4 {{ $columns === 2 ? 'lg:grid-cols-2' : '' }}">
            ...
            @foreach($column as $category)
                <div class="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_11rem] items-center gap-2">
                    ...
                    <div class="relative min-w-0 flex-1">
                        ...
                        <x-ui.input-control type="number" :id="$idPrefix.'-'.$category->id" :name="$prefix.'['.$category->id.']'" class="pl-7! text-right tabular-nums" min="0" :max="$percentage ? 100 : 10000000" step="0.01" x-model="values['{{ $category->id }}']" x-on:blur="format('{{ $category->id }}')" x-bind:disabled="!enabled" required />
                    </div>
                    <button type="button" class="inline-flex size-10 shrink-0 items-center justify-center rounded border border-slate-300 text-primary-color hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40" x-on:click="allocateRemaining('{{ $category->id }}')" x-bind:disabled="!enabled || remaining <= 0" aria-label="Allocate remaining {{ $percentage ? 'percentage' : 'amount' }} to {{ $category->name }}" title="Allocate remaining {{ $percentage ? 'percentage' : 'amount' }} to {{ $category->name }}">
    ...
    <dl class="{{ $columns === 2 ? 'lg:col-start-2' : '' }} space-y-2 border-t border-slate-200 pt-4 text-sm" aria-live="polite">
        ...
        <div class="flex justify-between gap-4" :class="remaining === 0 ? 'text-emerald-700' : 'text-amber-700'"><dt @if($shortfall) x-text="remaining < 0 ? 'Shortfall' : 'Unallocated'" @endif>Remaining</dt><dd class="font-semibold tabular-nums" x-text="money({{ $shortfall ? 'Math.abs(remaining)' : 'remaining' }})"></dd></div>
    </dl>
    @endif
</div>
