        ...
        <div class="space-y-3">
            @foreach($column as $category)
                <div class="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_11rem] items-center gap-2">
                    ...
                    <button type="button" class="inline-flex size-10 shrink-0 items-center justify-center rounded border border-slate-300 text-primary-color hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40" x-on:click="@if($editableDefaults) enabled = true; @endif allocateRemaining('{{ $category->id }}')" x-bind:disabled="{{ $editableDefaults ? 'remaining <= 0' : '!enabled || remaining <= 0' }}" aria-label="Allocate remaining {{ $percentage ? 'percentage' : 'amount' }} to {{ $category->name }}" title="Allocate remaining {{ $percentage ? 'percentage' : 'amount' }} to {{ $category->name }}">
