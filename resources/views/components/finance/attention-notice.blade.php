...
@endphp
@if($total > 0)
    <aside class="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900" aria-label="{{ $kind === 'product-stock' ? 'Product stock needing attention' : 'Finance items needing attention' }}">
        ...
        <div class="min-w-0">
            ...
            <div class="mt-2 flex flex-wrap gap-x-5 gap-y-2">
                ...
                @elseif($kind === 'product-stock')
                    <a data-dynamic-link class="underline underline-offset-2" href="{{ route('admin.shop.product.index', ['status_scope' => 'current', 'inventory' => 'actionable']) }}">{{ $total }} with low stock or orders awaiting fulfilment</a>
