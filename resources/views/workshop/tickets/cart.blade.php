    ...
    <x-container class="max-w-3xl mt-6 mx-auto">
        <div class="relative bg-white border border-gray-200 rounded-lg shadow-sm p-5 pt-20 md:pt-5 flex gap-6">
            ...
            <div class="mb-3 flex items-center gap-3">
                ...
                <h2 class="text-2xl font-bold">{{ $additionalWorkshops->isNotEmpty() ? 'Other workshops you may be interested in' : 'Your workshop booking' }}</h2>
            </div>
            <p class="mb-5 text-sm text-gray-600">Add another session for the same {{ $session['participant_count'] }} {{ $session['participant_count'] === 1 ? 'participant' : 'participants' }}, or continue with your booking. You’ll enter participant details once after checkout.</p>
            ...
            @if($additionalWorkshops->isNotEmpty())
                <div>
                    <p class="mb-4 text-sm text-gray-600">Check the times and allow for travel between venues. Online sessions can be added too.</p>
            ...
            @endif
            <details class="mt-6 border-t border-gray-200 pt-4" @if($additionalWorkshops->isEmpty()) open @endif>
                <summary class="cursor-pointer text-sm font-semibold">Your booking · {{ $checkoutWorkshops->count() }} {{ $checkoutWorkshops->count() === 1 ? 'workshop' : 'workshops' }} · {{ money($ticketPricing['subtotal_amount']) }}</summary>
