@foreach($workshopBookings as $booking)
    <div class="mb-4 rounded-xl border border-sky-200 bg-sky-50 p-4" x-data="SM.workshopHoldCountdown(@js($booking['expires_at']))" x-show="remainingSeconds > 0" x-cloak>
        ...
        <p class="mt-1 text-sm text-gray-600">{{ $booking['count'] }} {{ $booking['count'] === 1 ? 'ticket' : 'tickets' }} reserved · <span x-text="timeRemaining"></span> remaining</p>
