<div class="mb-1 flex items-center gap-2 pl-1">
    ...
    <div class="relative" x-data="{ placeholderHelpOpen: false }">
        <x-ui.button variant="plain" type="button" class="text-sky-700 hover:text-sky-900" x-on:click="placeholderHelpOpen = !placeholderHelpOpen" title="Show workshop placeholders" aria-label="Show workshop placeholders"><i class="fa-solid fa-circle-info"></i></x-ui.button>
        <div x-show="placeholderHelpOpen" x-cloak x-on:click.outside="placeholderHelpOpen = false" class="absolute left-0 top-7 z-20 w-[min(34rem,calc(100vw-3rem))] rounded-xl border border-sky-200 bg-white p-4 text-xs text-gray-700 shadow-xl">
            <div class="mb-3 flex items-center justify-between gap-3"><p class="text-sm font-semibold text-gray-900">Workshop placeholders</p><x-ui.button variant="plain" type="button" class="text-gray-500 hover:text-gray-800" x-on:click="placeholderHelpOpen = false" aria-label="Close placeholder help"><i class="fa-solid fa-xmark"></i></x-ui.button></div>
