@php
    ...
    $selectedOrganisations = $organisationOptions
        ...
        ->values();
    $selectedCategoryIds = collect(request()->query('category_ids', request()->filled('category_id') ? [request('category_id')] : []))
        ->map('strval')
        ->filter()
        ->values()
        ->all();
    $categoryOptions = $categories
        ->map(fn ($category) => ['id' => (string) $category->id, 'label' => (string) $category->name])
        ->values();
        ...
        <section>
            <div class="mb-4">
                @include('admin.workshop.partials.report-multi-picker', [
                    'pickerId' => 'matrix_category_search', 'fieldName' => 'category_ids', 'label' => 'Categories',
                    'placeholder' => 'Search and add categories', 'options' => $categoryOptions, 'selectedIds' => $selectedCategoryIds,
                ])
            </div>
            <div class="grid gap-x-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <x-ui.checkbox label="Past workshops only" name="past_only" value="1" :checked="request()->boolean('past_only')" class="lg:mt-7" />
