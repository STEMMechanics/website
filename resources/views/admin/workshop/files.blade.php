                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach($attachedFiles as $file)
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-3"><div class="flex min-w-0 items-center gap-3"><a href="{{ $file->download_url ?? (($file->url ?? '/media/'.rawurlencode((string) $file->name)).'?download=1') }}" target="_blank" class="shrink-0"><img src="{{ $file->thumbnail ?: asset('/thumbnails/unknown.webp') }}" onerror="this.onerror=null;this.src='{{ asset('/thumbnails/unknown.webp') }}';" alt="{{ $file->title }}" class="h-12 w-16 rounded bg-white object-contain p-1"></a><div class="min-w-0"><div class="font-medium text-gray-900">{{ trim((string) ($file->title ?? '')) !== '' ? $file->title : $file->name }}</div><div class="max-w-xs truncate text-xs text-gray-500">{{ $file->name }}</div><div class="text-xs text-gray-400"><x-ui.nonbreaking>{{ \App\Helpers::bytesToString((int) ($file->size ?? 0)) }}</x-ui.nonbreaking> · {{ $file->file_type }}</div><div class="md:hidden text-xs text-gray-500">Storage: <span class="capitalize">{{ $file->storageDiskName() }}</span></div><div class="md:hidden"><x-ui.badge :color="$fileVisibility === 'public' ? 'success' : ($fileVisibility === 'protected' ? 'warning' : 'slate')" class="capitalize">{{ $fileVisibility }}</x-ui.badge></div></div></div></td>
