...
<x-finance.panel title="Record drawing">
    <x-ui.select name="drawing_type" label="Drawing type" :value="$purpose" x-model="purpose" x-on:change="changeType()"><option value="time">Timesheet drawing</option><option value="contribution">Return of contributions</option></x-ui.select>
    <dl class="mb-4 space-y-2 text-sm">
        <div class="flex items-center justify-between gap-3"><dt class="text-slate-500">Outstanding to pay</dt><dd class="font-semibold tabular-nums" x-text="money(totals[purpose].outstanding)">{{ money(max(0, $earned - $paid) / 100) }}</dd></div>
        <div class="flex items-center justify-between gap-3"><dt class="text-slate-500">Available to pay</dt><dd class="font-semibold tabular-nums" x-text="money(totals[purpose].available)">{{ money(max(0, min($earned - $paid - $pending, $cash['available'])) / 100) }}</dd></div>
    </dl>
