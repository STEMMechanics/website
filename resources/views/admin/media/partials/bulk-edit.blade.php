<div class="px-5">
        <details class="group my-4 rounded-xl border border-gray-200 bg-white">
            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 font-semibold text-gray-900">
                <span>Editing {{ $selectedMedia->count() }} media {{ \Illuminate\Support\Str::plural('item', $selectedMedia->count()) }}</span>
                <i class="fa-solid fa-chevron-down text-sm text-gray-500 transition-transform group-open:rotate-180"></i>
            </summary>
            <div class="grid grid-cols-3 gap-3 border-t border-gray-200 p-4 sm:grid-cols-4">
                @foreach($selectedMedia as $medium)
                    <a href="{{ route('admin.media.edit', $medium) }}" target="_blank" rel="noopener noreferrer" class="group/item min-w-0">
                        <div class="flex aspect-square items-center justify-center overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                            <img loading="lazy" src="{{ $medium->thumbnail }}" alt="{{ $medium->title }}" class="h-full w-full object-contain">
                        </div>
        ...
        <form data-media-bulk-edit-form method="POST" action="{{ route('admin.media.bulk.update') }}">
            @csrf
            @method('PUT')
            @foreach($selectedMedia as $medium)<input type="hidden" name="media_names[]" value="{{ $medium->name }}">@endforeach
            ...
            <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4">
                ...
                <x-ui.grid class="gap-5 md:grid-cols-2">
                    ...
                    <x-ui.select label="Visibility" name="visibility" :value="$visibilityValue" class="mb-0">
                        ...
                        @if($visibilityValue === 'protected')<option value="__mixed" selected>Protected (unchanged)</option>@endif
                    </x-ui.select>
 ...
 />
                ...
                <div class="mt-2 max-h-64 overflow-y-auto rounded-lg border border-gray-200" x-show="search.trim().length >= 2" x-cloak>
                    <template x-for="workshop in results()" :key="workshop.id">
                        <x-ui.button variant="plain" type="button" x-on:click="add(workshop.id)" class="block w-full border-b border-gray-100 px-3 py-2 text-left last:border-0 hover:bg-sky-50">
