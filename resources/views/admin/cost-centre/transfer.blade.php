<x-layout><x-mast title="Transfer funds" backRoute="admin.cost-centre.index" backTitle="Cost centres" /><x-container class="py-5">
    <form x-data="{ source: @js(old('from_category_id', $from) ?? '') }" method="POST" action="{{ route('admin.cost-centre.transfer') }}" data-record-form>@csrf
        ...
        <x-ui.select name="category_id" label="To"><option value="">Choose a destination</option><option value="cash" x-bind:disabled="source === '' || source === 'remuneration'" @selected(old('category_id') === 'cash')>Available business cash</option>@foreach($categories->where('active', true) as $category)<option value="{{ $category->id }}" x-bind:disabled="source === '{{ $category->id }}'" @selected((string) old('category_id') === (string) $category->id)>{{ $category->name }}</option>@endforeach</x-ui.select>
