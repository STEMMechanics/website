...
<x-layout>
    <x-mast backRoute="admin.shop.category.index" backTitle="Product Categories">{{ isset($category) ? 'Edit' : 'Create' }} Product Category</x-mast>

    <x-container class="mt-4">
        <form
            id="product-category-form"
            method="POST"
            action="{{ route('admin.shop.category.'.(isset($category) ? 'update' : 'store'), $category ?? []) }}"
            x-data="{
        ...
        <x-ui.editor-actions>
            @if(isset($category))
                <form
                    method="POST"
                    action="{{ route('admin.shop.category.destroy', $category) }}"
                    x-data
                    x-on:submit.prevent="SM.confirmDelete('{{ csrf_token() }}', 'Delete category?', 'Are you sure you want to delete this category? Products must be detached first.', $el)"
                >
                    @csrf
                    @method('DELETE')
            ...
            <x-ui.button type="submit" form="product-category-form">Save</x-ui.button>
        </x-ui.editor-actions>
    </x-container>
</x-layout>
