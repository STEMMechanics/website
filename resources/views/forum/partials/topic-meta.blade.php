@php($topicSortQuery = !empty($topicSort ?? '') ? ['topicSort' => $topicSort] : [])
<div class="-mt-4 flex flex-col mb-4">
    <div class="flex flex-col sm:flex-row gap-3">
        @auth
            <form method="POST" action="{{ route('forum.topic.notifications', array_merge(['categorySlug' => $category->slug, 'topicSlug' => $topic->slug, 'sort' => $replySort ?? 'oldest'], $topicSortQuery)) }}" class="flex items-center gap-3" data-forum-notifications-form>
                @csrf
            ...
            @if(auth()->user()?->isAdmin() || (string) $topic->user_id === (string) auth()->id())
                @if(auth()->user()?->isAdmin())
                    <div class="flex gap-3">
                        <form class="flex-1 flex" method="POST" action="{{ route('forum.topic.pin', array_merge(['categorySlug' => $category->slug, 'topicSlug' => $topic->slug], $topicSortQuery)) }}">
                            @csrf
                        ...
                        <form class="flex-1 flex" method="POST" action="{{ route('forum.topic.lock', array_merge(['categorySlug' => $category->slug, 'topicSlug' => $topic->slug], $topicSortQuery)) }}">
                            @csrf
                ...
                @endif
                <form class="w-full sm:w-auto" method="POST" action="{{ route('forum.topic.destroy', array_merge(['categorySlug' => $category->slug, 'topicSlug' => $topic->slug], $topicSortQuery)) }}" x-data x-on:submit.prevent="SM.confirmDelete('{{ csrf_token() }}', 'Delete thread?', 'Are you sure you want to delete this thread and all replies?', $el)">
                    @csrf
                    @method('DELETE')
