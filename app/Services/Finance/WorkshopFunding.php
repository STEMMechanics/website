...
class WorkshopFunding
{
    ...
    {
        if ($line->kind === 'workshop') return ! empty($line->details_json['workshop']['linked_workshop_id']) ? collect([$line]) : collect();
        $entries = collect();
        if ($line->kind !== 'multi_workshop') return $entries;
        foreach ($line->details_json['multi_workshop']['rows'] ?? [] as $index => $row) {
            $settings = $row['details_json']['workshop'] ?? [];
            if (empty($settings['linked_workshop_id'])) continue;
            $entry = clone $line;
    ...
    public function linkedLines(?array $workshopIds = null): Collection
    {
        $lines = InvoiceLine::with('invoice')->where(function ($query) use ($workshopIds) {
            $query->where(function ($single) use ($workshopIds) {
                ...
                if ($workshopIds !== null) $single->whereIn('details_json->workshop->linked_workshop_id', $workshopIds);
    ...
    public static function settings(array $settings): array
    {
        $data = Validator::make($settings, [
            ...
            'allocation_basis' => 'nullable|in:manual,capacity,tickets',
            'allocation_seats' => 'nullable|integer|min:0|max:10000',
        ])->validate();
        if (empty($data['linked_workshop_id'])) return [];
    ...
    public function validateLinks(Invoice $invoice): void
    {
        ...
        foreach ($invoice->lines()->whereIn('kind', ['workshop', 'multi_workshop'])->get()->flatMap(fn ($line) => $this->entries($line)) as $line) {
            ...
            if (! $id || $invoice->status === Invoice::STATUS_CANCELLED) continue;
            if (isset($seen[$id]) || $this->lines($id)->contains(fn ($other) => $other->invoice_id !== $invoice->id && $other->invoice->status !== Invoice::STATUS_CANCELLED)) {
                throw \Illuminate\Validation\ValidationException::withMessages(['workshop_funding' => 'This workshop already has a funding line. Use one funding line per workshop so its allocation settings are unambiguous.']);
            }
            $seen[$id] = true;
        }
    }
        ...
        foreach ($input as $id => $settings) {
            ...
            if ($line->kind === 'multi_workshop') {
                foreach ($settings['rows'] ?? [] as $index => $rowSettings) {
                    if (! isset($details['multi_workshop']['rows'][$index])) abort(422, 'Unknown workshop row.');
            ...
            foreach (['linked_workshop_id', 'allocation_basis', 'allocation_seats'] as $field) unset($current[$field]);
    ...
    {
        return Workshop::with('location')->withCount(['tickets as registered_ticket_count' => fn ($query) => $query->whereIn('status', Ticket::activePurchasedStatuses())])->orderByDesc('starts_at')->get()->map(fn ($workshop) => [
            'id' => (string) $workshop->id,
            'label' => $workshop->title.' · '.($workshop->starts_at?->format('d M Y') ?? 'Date TBC').' · '.$workshop->getLocationName(),
            'title' => $workshop->title, 'date' => $workshop->starts_at?->format('Y-m-d'),
            'hours' => $workshop->teachingHours(), 'capacity' => $workshop->max_tickets, 'tickets' => $workshop->getAttribute('registered_ticket_count'),
        ])->all();
    }
}
