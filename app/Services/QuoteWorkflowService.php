    ...
    public function createInvoiceFromQuote(Quote $quote, bool $issueInvoice = false): Invoice
    {
        ...
        foreach ($sourceLineItems as $index => $lineItem) {
            ...
            $invoice->lines()->create([
                ...
                'source_type' => ($lineItem['kind'] ?? '') === 'product' && ! empty($lineItem['source_id']) ? \App\Models\Product::class : null,
                'source_id' => ($lineItem['kind'] ?? '') === 'product' ? ($lineItem['source_id'] ?? null) : null,
                'details_json' => array_merge($lineItem['details_json'] ?? [], ($lineItem['kind'] ?? '') === 'product' ? ['variant_id' => $lineItem['source_variant_id'] ?? data_get($lineItem, 'store_context.variant_id')] : []),
                'quantity' => $quantity,
