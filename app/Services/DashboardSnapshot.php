...
class DashboardSnapshot
{
    ...
    public function get(string $period): array
    {
        ...
        $key = 'dashboard-snapshot:v7:'.config('app.timezone').':'.now()->toDateString().':'.$period;
        try {
    ...
    public function refresh(string $period): array
    {
        ...
        try {
            Cache::put('dashboard-snapshot:v7:'.config('app.timezone').':'.now()->toDateString().':'.$period, $payload, max(1, (int) config('analytics.dashboard_snapshot_seconds', 300)));
        } catch (\Throwable) {
    ...
    private function decode(string $payload): array
    {
        ...
        $data['mediaDownloadRows'] = collect($data['mediaDownloadRows'])->map(fn (array $row) => (object) $row);
