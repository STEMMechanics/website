...
class NewsletterPersonalNoteTest extends TestCase
{
    ...
    public function test_note_is_saved_rendered_between_header_and_items_and_snapshotted_before_reset(): void
    {
        ...
        $this->put(route('admin.subscription.store-promotion.update'), [
            ...
            'personal_note' => ['enabled' => '1', 'body' => $body],
        ...
        $this->get(route('admin.newsletter.index'))->assertOk()->assertSee('Edit personal note')->assertSee('A little update from me.');
    ...
    public function test_disabled_note_is_omitted_and_an_enabled_note_requires_text(): void
    {
        $selector = app(NewsletterProductSelectionService::class);
        $draft = $selector->draft();
        $this->put(route('admin.subscription.store-promotion.update'), ['sections' => $draft->sections, 'personal_note' => ['enabled' => 1, 'body' => '']])->assertSessionHasErrors('personal_note.body');
        ...
        $this->assertStringNotContainsString('Keep this draft', (new UpcomingWorkshops('test@example.com'))->render());
    ...
    public function test_rich_text_preserves_formatting_and_links_but_removes_unsafe_markup(): void
    {
        ...
        $this->put(route('admin.subscription.store-promotion.update'), ['sections' => $draft->sections, 'personal_note' => ['enabled' => 1, 'format' => 'html', 'body' => '<p><br></p>']])->assertSessionHasErrors('personal_note.body');
    }
