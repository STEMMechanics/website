...
class ForumContentFilterTest extends TestCase
{
    ...
    public function test_forum_topic_creation_honours_obfuscated_exception_words(): void
    {
        $user = User::factory()->create();
        $category = ForumCategory::query()->create([
            'name' => 'General Discussion',
        ...
        $response = $this->actingAs($user)->post(route('forum.topic.store', $category->slug), [
            'title' => 'Need help',
            'body' => '<p>This contains F:49 but should be allowed because it is on the exception list.</p>',
        ]);
